...
export class PurchaseEventHandler implements EventHandler<any> {
  ...
  async handle(event: any): Promise<void> {
    switch (event.eventType) {
      ...
      case 'UserValidationFailed':
        await this.handleUserValidationFailed(event as UserValidationFailedEvent);
        break;
      case 'ItemGranted':
      ...
      case 'CompensationInitiated':
        await this.handleCompensationInitiated(event as CompensationInitiatedEvent);
        break;
      case 'CompensationCompleted':
        await this.handleCompensationCompleted(event as CompensationCompletedEvent);
        break;
  ...
  private async handleUserValidationFailed(event: UserValidationFailedEvent): Promise<void> {
    this.logger.warn(`User validation failed: ${event.userId} (Transaction: ${event.transactionId})`);
    this.logger.debug(`Failure reason: ${event.reason} (Code: ${event.errorCode})`);
  }

  private async handleItemGranted(event: ItemGrantedEvent): Promise<void> {
    this.logger.log(`Item granted: ${event.itemId} x${event.quantity} to user ${event.userId} (Transaction: ${event.transactionId})`);
  }

  private async handleItemGrantFailed(event: ItemGrantFailedEvent): Promise<void> {
    this.logger.warn(`Item grant failed: ${event.itemId} for user ${event.userId} (Transaction: ${event.transactionId})`);
    this.logger.debug(`Failure reason: ${event.reason} (Code: ${event.errorCode})`);
  }

  private async handleLogRecorded(event: LogRecordedEvent): Promise<void> {
    this.logger.log(`Log recorded: ${event.logId} (Transaction: ${event.transactionId})`);
    this.logger.debug(`Log data: ${JSON.stringify(event.logData)}`);
  }

  private async handleLogFailed(event: LogFailedEvent): Promise<void> {
    this.logger.error(`Log recording failed (Transaction: ${event.transactionId})`);
    this.logger.debug(`Failure reason: ${event.reason} (Code: ${event.errorCode})`);
  }

  private async handleNotificationSent(event: NotificationSentEvent): Promise<void> {
    this.logger.log(`Notification sent: ${event.notificationId} to user ${event.userId} (Transaction: ${event.transactionId})`);
  ...
  private async handleNotificationFailed(event: NotificationFailedEvent): Promise<void> {
    this.logger.warn(`Notification failed for user ${event.userId} (Transaction: ${event.transactionId})`);
    this.logger.debug(`Failure reason: ${event.reason} (Code: ${event.errorCode})`);
  }

  private async handlePurchaseCompleted(event: PurchaseCompletedEvent): Promise<void> {
    this.logger.log(`🎉 Purchase completed successfully: ${event.transactionId}`);
    this.logger.log(`User ${event.userId} received ${event.quantity} x ${event.itemId} for ${event.totalPrice}`);
  ...
  private async handlePurchaseFailed(event: PurchaseFailedEvent): Promise<void> {
    this.logger.error(`❌ Purchase failed: ${event.transactionId}`);
    this.logger.error(`User: ${event.userId}, Item: ${event.itemId}, Failed at: ${event.failedStep}`);
    this.logger.error(`Failure reason: ${event.failureReason} (Code: ${event.errorCode})`);
    
    // 실패 메트릭 수집
    this.collectFailureMetrics(event);
  }

  private async handleCompensationInitiated(event: CompensationInitiatedEvent): Promise<void> {
    this.logger.log(`🔄 Compensation initiated: ${event.transactionId}`);
    this.logger.debug(`Compensation type: ${event.compensationType}, Original failed step: ${event.originalFailedStep}`);
  }

  private async handleCompensationCompleted(event: CompensationCompletedEvent): Promise<void> {
    this.logger.log(`✅ Compensation completed: ${event.transactionId}`);
    this.logger.debug(`Compensation details: ${JSON.stringify(event.compensationDetails)}`);
  }

  private async handleCompensationFailed(event: CompensationFailedEvent): Promise<void> {
    this.logger.error(`💥 Compensation failed: ${event.transactionId}`);
    this.logger.error(`Compensation type: ${event.compensationType}`);
    this.logger.error(`Failure reason: ${event.reason} (Code: ${event.errorCode})`);
  ...
  private collectSuccessMetrics(event: PurchaseCompletedEvent): void {
    ...
    this.logger.debug(`Metrics: Purchase success - User: ${event.userId}, Item: ${event.itemId}, Price: ${event.totalPrice}`);
  }

  private collectFailureMetrics(event: PurchaseFailedEvent): void {
    // 실제 환경에서는 메트릭 수집 시스템으로 전송
    this.logger.debug(`Metrics: Purchase failure - Step: ${event.failedStep}, Error: ${event.errorCode}`);
  }

  private async handleCriticalCompensationFailure(event: CompensationFailedEvent): Promise<void> {
    ...
    this.logger.error(`🚨 CRITICAL: Manual intervention required for compensation failure`);
    this.logger.error(`Transaction: ${event.transactionId}, Type: ${event.compensationType}`);
