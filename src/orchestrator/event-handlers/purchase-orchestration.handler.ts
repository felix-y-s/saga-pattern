...
@Injectable()
export class PurchaseOrchestrationHandler implements EventHandler<PurchaseInitiatedEvent> {
  ...
  async handle(event: PurchaseInitiatedEvent): Promise<void> {
    try {
      this.logger.log(`🚀 Event-driven purchase initiated: ${event.transactionId}`);
      this.logger.debug(`Processing purchase for user ${event.userId}, item ${event.itemId}`);
      ...
      if (result.success) {
        this.logger.log(`✅ Event-driven purchase completed: ${event.transactionId}`);
        this.logger.debug(`Completed steps: ${result.completedSteps.join(', ')}`);
      } else {
        this.logger.warn(`❌ Event-driven purchase failed: ${event.transactionId}`);
        if (result.error) {
          this.logger.debug(`Failed at step: ${result.error.step}, reason: ${result.error.message}`);
        }
      }
      
    } catch (error) {
      this.logger.error(`💥 Event-driven purchase handler error: ${event.transactionId}`, error);
  ...
  private async handleCriticalError(event: PurchaseInitiatedEvent, error: any): Promise<void> {
    this.logger.error(`🚨 CRITICAL: Event handler failure requires manual intervention`);
    this.logger.error(`Transaction: ${event.transactionId}, User: ${event.userId}, Item: ${event.itemId}`);
