...
export class UserService {
  ...
  private readonly users = new Map<string, UserProfile>([
    ...
    [
      'user-456',
      { userId: 'user-456', balance: 50, status: 'active', purchaseLimit: 100 },
    ],
    [
      'user-suspended',
      {
  ...
  async validateUser(dto: UserValidationDto): Promise<UserValidationResult> {
    this.logger.debug(
      `Validating user: ${dto.userId} for transaction: ${dto.transactionId}`,
    );
    ...
    try {
      ...
      if (user.status !== 'active') {
        this.logger.warn(
          `User not active: ${dto.userId}, status: ${user.status}`,
        );
        return {
          isValid: false,
          userId: dto.userId,
      ...
      if (user.balance < dto.requiredBalance) {
        this.logger.warn(
          `Insufficient balance: ${dto.userId}, required: ${dto.requiredBalance}, current: ${user.balance}`,
        );
        return {
          isValid: false,
          userId: dto.userId,
      ...
      if (dto.requiredBalance > user.purchaseLimit) {
        this.logger.warn(
          `Purchase limit exceeded: ${dto.userId}, limit: ${user.purchaseLimit}, required: ${dto.requiredBalance}`,
        );
        return {
          isValid: false,
          userId: dto.userId,
      ...
      this.logger.log(
        `User validation successful: ${dto.userId}, remaining balance: ${user.balance}`,
      );
  ...
  async compensateUserValidation(
    ...
    this.logger.debug(
      `Compensating user validation: ${userId}, amount: ${amount}, transaction: ${transactionId}`,
    );
    ...
    try {
      ...
      user.balance += amount;
      this.logger.log(
        `User validation compensated: ${userId}, restored balance: ${user.balance}`,
      );
