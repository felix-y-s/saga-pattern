...
export class CompensationHandler
  implements
    EventHandler<
      UserValidationFailedEvent | ItemGrantFailedEvent | LogFailedEvent
    >
{
  ...
  async handle(
    event: UserValidationFailedEvent | ItemGrantFailedEvent | LogFailedEvent,
  ): Promise<void> {
    const { transactionId } = event;

    this.logger.log(
      `🔄 Starting compensation for transaction: ${transactionId}`,
    );
    ...
    try {
      ...
      if (!sagaState) {
        this.logger.error(
          `Saga state not found for compensation: ${transactionId}`,
        );
        return;
      }
      ...
      this.logger.debug(
        `Compensating steps: ${successfulSteps.join(', ')} for ${transactionId}`,
      );
      ...
      this.logger.log(
        `✅ Compensation completed successfully: ${transactionId}`,
      );
    } catch (error) {
   ...
   * 보상이 필요한지 판단
   */
  private shouldCompensate(
    context: SagaContextImpl,
    failureEvent: any,
  ): boolean {
    // UserValidationFailed의 경우 보상 불필요 (아직 아무것도 실행하지 않음)
    if (
      failureEvent instanceof UserValidationFailedEvent ||
      failureEvent.eventType === 'UserValidationFailed'
    ) {
      return false;
    }
  ...
  private async executeCompensationStep(
    ...
    try {
      ...
      this.logger.debug(
        `✅ Compensation successful for step ${step}: ${context.state.transactionId}`,
      );
    } catch (error) {
  ...
  private async markPurchaseAsFailed(
    ...
    try {
      ...
      this.logger.log(
        `❌ Purchase marked as failed: ${transactionId} - ${errorInfo.message}`,
      );
    } catch (error) {
      this.logger.error(
        `💥 Failed to mark purchase as failed: ${transactionId}`,
        error,
      );
    }
  }
   ...
   */
  private extractErrorInfo(event: any): {
    step: SagaStep;
    code: string;
    message: string;
  } {
  ...
  private async handleCriticalCompensationFailure(
    ...
    this.logger.error(
      `Transaction: ${transactionId}, Original failure: ${originalEvent.eventType}`,
    );
    ...
    try {
      ...
      this.logger.error(
        `💥 Failed to mark purchase as failed after compensation failure: ${transactionId}`,
        markingError,
      );
    }
  }
