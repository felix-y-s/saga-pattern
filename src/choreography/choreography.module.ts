...
import { isChoreographyMode, getSagaPatternConfig } from '../config/saga-pattern.config';
...
export class ChoreographyModule implements OnModuleInit {
  constructor(private moduleRef: ModuleRef) {
    console.log('🎭 ChoreographyModule initialized - Event-driven saga pattern ready');
  }
  ...
  async onModuleInit() {
    ...
    console.log(`🔧 ChoreographyModule: Current saga pattern mode = ${config.mode}`);

    // 코레오그래피 모드일 때만 핸들러 등록
    if (!isChoreographyMode()) {
      console.log('⏸️ Choreography handlers DISABLED (Orchestration mode active)');
      return;
    }
    ...
    if (!eventBus) {
      throw new Error('EventBusService not found. Make sure EventBusModule is imported.');
    }

    console.log('🎭 CHOREOGRAPHY MODE: Activating event handlers...');

    // 🔍 사용자 검증 핸들러: PurchaseInitiated 이벤트 구독
    const userValidationHandler = this.moduleRef.get(UserValidationHandler, { strict: false });
    eventBus.subscribe('PurchaseInitiated', userValidationHandler);
    
    // 📦 아이템 지급 핸들러: UserValidated 이벤트 구독
    const itemGrantHandler = this.moduleRef.get(ItemGrantHandler, { strict: false });
    eventBus.subscribe('UserValidated', itemGrantHandler);
    
    // 📝 로그 기록 핸들러: ItemGranted 이벤트 구독  
    const logRecordHandler = this.moduleRef.get(LogRecordHandler, { strict: false });
    eventBus.subscribe('ItemGranted', logRecordHandler);
    
    // 📢 알림 핸들러: LogRecorded 이벤트 구독
    const notificationHandler = this.moduleRef.get(NotificationHandler, { strict: false });
    eventBus.subscribe('LogRecorded', notificationHandler);
    
    // 🔄 보상 핸들러: 실패 이벤트들 구독
    const compensationHandler = this.moduleRef.get(CompensationHandler, { strict: false });
    ...
    console.log('✅ CHOREOGRAPHY MODE: All event handlers registered successfully');
    console.log('📋 Choreography event subscription summary:');
    console.log('  - UserValidationHandler → PurchaseInitiated (BUSINESS LOGIC)');
    ...
    console.log('  - CompensationHandler → UserValidationFailed, ItemGrantFailed, LogFailed');
